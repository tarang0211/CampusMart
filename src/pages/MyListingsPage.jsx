...
export const MyListingsPage = () => {
  const { products, fetchMyProducts, markAsSold, deleteProduct, editProduct } =
    useProducts();
  ...
  useEffect(() => {
    const loadMyListings = async () => {
      ...
      try {
        ...
        showToast(error.message || "Failed to load your listings", "error");
      } finally {
        setLoading(false);
      }
    };
  ...
  const activeCount = userListings.filter((product) => !product.isSold).length;

  const soldCount = userListings.filter((product) => product.isSold).length;
  ...
  const handleToggleSold = async (id, currentStatus, title) => {
    try {
      ...
      if (!currentStatus) {
        ...
        showToast("Listing re-activated for sale", "info");
      }
    } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showToast("Listing deleted from BitMart", "info");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 dark:border-slate-800 pb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage your posted items, update prices, or mark items as sold.
          </p>
        </div>

        <Link to="/sell">
          <Button variant="primary" icon={PlusCircle} className="font-bold">
            Post New Item
          </Button>
        </Link>
      </div>
      ...
      >
        <div className="space-y-4 text-center">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Are you sure you want to delete this listing from BitMart? This
